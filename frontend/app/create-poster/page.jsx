...
export default function CreatePoster() {
  ...
  const generate = () => {
    setStatus("Brief ready — connect GEMINI_API_KEY to run AI analysis.");
  };

  return <main className="a2z-shell min-h-screen text-white">
    <header className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6"><a href="/" className="font-bold">A2ZCreate<span className="text-violet-400">.</span></a><a href="/dashboard" className="text-sm text-zinc-400 hover:text-white">← Dashboard</a></header>
    <section className="mx-auto max-w-7xl px-6 pb-20 pt-8">
      <div className="max-w-3xl"><p className="text-xs uppercase tracking-[.2em] text-violet-400">AI poster studio</p><h1 className="mt-3 text-4xl font-semibold tracking-tight md:text-5xl">Create a poster from your idea — or your inspiration.</h1><p className="mt-4 leading-7 text-zinc-400">Upload an inspiration poster, your product and logo. A2ZCreate uses the reference for visual direction while keeping your content, branding and product in control.</p></div>
      <div className="mt-10 grid gap-5 lg:grid-cols-3">
        <UploadBox label="Inspiration poster" hint="PNG, JPG, WEBP" file={inspiration} onChange={setInspiration} accept="image/*" accent="text-violet-300" />
        <UploadBox label="Product image" hint="Use the exact product you want shown" file={product} onChange={setProduct} accept="image/*" accent="text-emerald-300" />
        <UploadBox label="Brand logo" hint="Transparent PNG recommended" file={logo} onChange={setLogo} accept="image/*" accent="text-blue-300" />
      </div>

      <div className="mt-5 grid gap-5 lg:grid-cols-[1.35fr_.65fr]">
        <section className="a2z-card p-6"><div className="mb-6"><h2 className="text-xl font-semibold">Creative brief</h2><p className="mt-1 text-sm text-zinc-500">Tell the AI what the poster needs to communicate.</p></div>
          ...
          <div className="mt-5"><label className="a2z-label">Poster script / message</label><textarea className="a2z-input min-h-36 resize-y" value={script} onChange={e=>setScript(e.target.value)} placeholder="Write the headline, offer, supporting text, CTA or simply describe what you want..." /></div>
          <div className="mt-5 grid gap-5 md:grid-cols-2"><div><label className="a2z-label">Visual theme</label><select className="a2z-input" value={theme} onChange={e=>setTheme(e.target.value)}>{['Modern','Luxury','Minimal','Corporate','Bold','Editorial','Festival'].map(x=><option key={x}>{x}</option>)}</select></div><div><label className="a2z-label">Footer</label><button type="button" onClick={()=>setFooter(!footer)} className={`a2z-input text-left ${footer?'border-white/25':''}`}>{footer?'✓ Footer enabled':'○ Footer disabled'}<span className="ml-2 text-xs text-zinc-500">Website • phone • CTA • QR</span></button></div></div>
          <button onClick={generate} className="a2z-btn a2z-btn-primary mt-7 w-full">Analyze & create poster →</button>{status && <p className="mt-4 rounded-xl border border-violet-400/20 bg-violet-400/5 p-3 text-sm text-violet-200">{status}</p>}
        </section>

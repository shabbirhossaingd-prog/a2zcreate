...
export default function CreateVideo() {
  ...
  async function generate() {
    ...
    try {
      ...
      setStatus("Gemini scene plan generated. These scenes are ready for voice, clips and render queue.");
    } catch (e) {
      setStatus(e.message);
    } finally {
      setBusy(false);
    ...
    <section className="mx-auto max-w-7xl px-6 pb-20 pt-8">
      <div className="grid gap-8 lg:grid-cols-[1fr_.78fr] lg:items-end">
        <div className="max-w-3xl">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[.04] px-3 py-1 text-xs text-zinc-300"><span className="h-1.5 w-1.5 rounded-full bg-violet-400"/> Scene based video maker</div>
          <h1 className="text-5xl font-semibold leading-tight tracking-[-.04em] md:text-6xl">Script in. <span className="text-zinc-500">Scenes out.</span></h1>
          <p className="mt-5 leading-7 text-zinc-400">A2ZCreate breaks long videos into short 5–10 second shots, then prepares voice, music, subtitles and brand instructions for the final render.</p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-3">
        <div className="a2z-card a2z-card-hover p-5"><Icon name="voice"/><p className="mt-5 text-xs text-zinc-500">VOICE</p><p className="mt-2 font-medium">ElevenLabs voiceover</p><p className="mt-1 text-sm text-zinc-500">Generate narration per scene.</p></div>
